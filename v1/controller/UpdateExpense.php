...
try {
    $insertQuery = $writeDb->prepare('UPDATE `expense` SET `date`=:date,`category`=:category,`price`=:price,`notes`=:notes,`modifiedby`=:modifiedby WHERE `id`=:id');
    ...
    if ($insertQuery->rowCount() === 0) {
        UtilityFunction::sendResponse(false, 500, "Trouble updating expense. Please try again later.");
        exit;
    }
    UtilityFunction::sendResponse(true, 201, "Expense Updated Successfully");
    exit;
} catch (PDOException $ex) {
    $writeDb->rollBack();
    error_log("Database query error " . $ex, 0);
    UtilityFunction::sendResponse(false, 500, "Trouble updating expense. Please try again later. " . $ex);
    exit;
}
